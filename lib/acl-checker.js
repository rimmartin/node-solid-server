...
class ACLChecker {
  ...
  async can (user, mode) {
    ...
    if (this.isAcl(path)) {
      console.log('DAAAAAAAAAAAHUT')
      modesRequired.push(ACL('Control'))
    } else {
      console.log('DAAAAAAHUT')
    ...
    do {
      try {
        ...
        if (error.code === 'ENOENT') {
          ...
          this.reject('A 500 of some sort')
        }
      }
      ...
      if (path === root) {
        this.reject('Server has been misconfigured: No root ACL') // various other parameters
      }

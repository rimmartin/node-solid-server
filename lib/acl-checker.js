...
class ACLChecker {
  constructor (resource, options = {}) {
    ...
    this.host = 'tim.localhost' // options.host
    this.origin = options.origin ? $rdf.sym(options.origin) : null
    ...
    let tmpOrigins = [] // Not sure why...
    if (options.trustedOrigins) {
      options.trustedOrigins.forEach(function (element) {
        tmpOrigins.push($rdf.sym(element))
      })
    }
    this.trustedOrigins = tmpOrigins
  }
  ...
  async can (user, mode) {
    ...
    console.log('Checking permissions for ' + this.resource)
    ...
    if (!isContainer) {
      ...
      if (aclText) {
        ...
        try {
          console.log(url)
          $rdf.parse(aclText, kb, url.url, url.contentType || 'text/turtle')
        } catch (error) {
          console.log(error)
          throw new HTTPError(500, 'Server Error: Failed to parse ACL file: ' + error.message)
        }
        console.log(this.trustedOrigins)
        var deny
        try {
          deny = aclCheck.accessDenied(kb, doc, null, $rdf.sym('https://tim.localhost:7777/read-acl/.acl'), agent, modesRequired, this.origin, this.trustedOrigin)
        } catch (error) {
        ...
        console.log(deny)
        if (deny) {
          this.reject(user, deny) // With various parameters
        }
        return true
      }
    ...
    const url = await this.mapper.mapFileToUrl({path: filename, hostname: this.host})
    const dir = await this.mapper.mapFileToUrl({path: path, hostname: this.host})
    try {
      $rdf.parse(aclText, kb, url.url, 'text/turtle')
    } catch (error) {
      throw new HTTPError(500, 'Server Error: Failed to parse ACL file: ' + error.message)
    }

    const denied = aclCheck.accessDenied(kb, doc, dir, url, agent, modesRequired, $rdf.sym(this.origin), this.trustedOrigins)
    if (denied) {

...
class LDP {
  ...
  async _put (host, resourcePath, callback, resourceHandler) {
    const ldp = this
    ...
    if (filePath.endsWith('/')) {
      return callback(error(405, 'PUT not supported on containers, use POST instead'))
    }

    // First check if we are above quota
    const isOverQuota = await overQuota(root, this.serverUri)
    if (isOverQuota) {
      return callback(error(413, 'User has exceeded their storage quota'))
    }

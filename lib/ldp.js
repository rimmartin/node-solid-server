...
class LDP {
  ...
  async readContainerMeta (directory) {
    const ldp = this
    ...
    return new Promise((resolve, reject) => {
      ldp.readFile(directory + ldp.suffixMeta, function (err, data) {
        if (err) {
          const errorObject = error(err, "Can't read meta file")
  ...
  get (options, callback) {
    ...
    ldp.stat(filename, async function (err, stats) {
      // File does not exist
      if (err) {
      ...
      if (stats.isDirectory()) {
        let metaFile
        try {
          metaFile = await ldp.readContainerMeta(filename)
        } catch (err) {
          metaFile = ''
        }
        const absContainerUri = baseUri + reqPath
        ldp.listContainer(filename, absContainerUri, baseUri, metaFile, contentType, function (err, data) {
          if (err) {
          ...
          // TODO 'text/turtle' is fixed, should be contentType instead
          // This forces one more translation turtle -> desired
          return callback(null, {stream, 'contentType': 'text/turtle', 'container': true})
        })
        return
      } else {
        let stream
